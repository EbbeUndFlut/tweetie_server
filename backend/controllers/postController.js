...
const createPost = async (req, res) => {
    ...
    console.log(post)
    if (parentPostId) await Post.findOneAndUpdate({ _id: parentPostId }, { $inc: { comments: 1 } })
    if (post) {
        res.status(201).end()
    } else {
        res.status(500).end()
...
const getConversation = async (req, res) => {
    ...
    const post = await Post.findOne({ _id: ObjectId(parentId) }).populate("creator")
    console.log("DER POST:", post)
...
const setAsFav = async (req, res) => {
    ...
    if (isFav) {
        await Post.updateOne({ _id: ObjectId(favId) }, { $inc: { likes: 1 } })

        const result = await Fav.updateOne({ user: userId }, { $push: { favorites: [ObjectId(favId)] } })
...
const getFavs = async (req, res) => {
    ...
    const result = await Fav.find({ user: ObjectId(user) }).populate("favorites")
